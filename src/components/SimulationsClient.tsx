...
export default function SimulationsClient() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (mounted && json?.simulations) setSimulations(json.simulations)
    }

    load()

    const es = new EventSource('/api/simulations/stream')
    ...
    es.onmessage = (ev) => {
      try {
        ...
        if (payload.type === 'created') {
          setSimulations((s) => [payload.simulation, ...s])
        } else if (payload.type === 'updated') {
          setSimulations((s) => s.map((it) => (it.id === payload.simulation.id ? payload.simulation : it)))
        } else if (payload.type === 'deleted') {
          setSimulations((s) => s.filter((it) => it.id !== payload.id))
        }
      } catch (e) {
        // ignore parse errors
      }
    ...
    es.onerror = () => {
      // reconnect handled by EventSource automatically in browsers
    }
